...
$idsiswa = dekripsi($_GET['id']);
$siswa = fetch($koneksi, 'siswa', ['id' => $idsiswa]);
...
<body style="font-size: 13px;">
    ...
    <hr>
    <center>
        <h3>TRANSKIP NILAI <?= $setting['nama_sekolah'] ?></h3>
    </center>
            ...
            <tbody>
                <?php if ($skl['kelompok'] == 1) { ?>
                    ...
                    while ($kelompok = mysqli_fetch_array($q1)) {
                        ...
                        $query = mysqli_query($koneksi, "SELECT * FROM mapel where kelompok='$kelompok[kelompok]' and aktif_transkip='1' and (jurusan='$siswa[jurusan]' or jurusan='semua') order by no_urut ");

                    ?>
                        <tr>
                            <td colspan="8"><b><?= $kelompok['kelompok'] ?></b></td>
                        </tr>
                        <?php
                        ...
                        ?>
                            <tr>
                                ...
                                <?php for ($i = 1; $i < 7; $i++) { ?>
                                    <?php $nilai = fetch($koneksi, 'nilai', ['kode_mapel' => $mapel['kode_mapel'], 'nis' => $siswa['nis'], 'semester' => $i]);
                ...
                <?php } else { ?>
                    ...
                    <?php
                    $query = mysqli_query($koneksi, "SELECT * FROM mapel where (jurusan='$siswa[jurusan]' or jurusan='semua') and aktif_transkip='1' order by no_urut ");
                    $no = 0;
                    $total = 0;
                    while ($mapel = mysqli_fetch_array($query)) {
        ...
        <table width="100%">
            <tr>
                ...
                <td style="text-align: center;width:180px">
                    <?php if ($skl['foto'] == 1) { ?>
                        <img width="100" class="img" src="../mod_photo/upload/<?= $siswa['nis'] ?>.JPG">
                    <?php } ?>
